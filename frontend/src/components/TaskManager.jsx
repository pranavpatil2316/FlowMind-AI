...
const TaskManager = () => {
  const { tasks, addTask, updateTask, deleteTask, toggleTask, searchQuery, productivityScore } = useTasks();
  ...
  return (
    <div className="max-w-6xl mx-auto animate-slide-up h-full text-left">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
        ...
        {/* Right Side: Productivity Dashboard Insights Widgets (Option A / Sidebar density) */}
        <div className="space-y-6">
          {/* Widget 1: Productivity Quotient */}
          <div className="glass-card p-5 rounded-2xl border border-white/5 space-y-4 shadow-lg">
            <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block">Productivity Score</span>
            <div className="flex items-baseline gap-2">
              <span className="text-4xl font-extrabold text-white tracking-tight">{productivityScore}%</span>
            ...
            <p className="text-[10px] text-slate-400 leading-normal font-semibold">
              Weight-based indicator calculated using your task checklists. Keep completing tasks to reach 100%!
            </p>
          </div>

          {/* Widget 2: Today's Focus */}
          <div className="glass-card p-5 rounded-2xl border border-white/5 space-y-4 shadow-lg">
            <span className="text-[10px] font-extrabold text-slate-500 uppercase tracking-wider block">Today's Focus</span>
            <div className="space-y-3">
              {tasks.filter(t => t.status === 'Pending' && t.priority === 'High').length === 0 ? (
                <p className="text-[10px] text-slate-500 font-semibold italic">No pending High-priority tasks today! Good work.</p>
              ) : (
                tasks.filter(t => t.status === 'Pending' && t.priority === 'High').map(task => (
                  <div key={task.id} className="flex items-center gap-2.5 text-xs font-semibold text-slate-300">
          ...
          <div className="glass-card p-5 rounded-2xl border border-white/5 space-y-4 shadow-lg">
            <div className="flex items-center gap-1.5 border-b border-white/[0.04] pb-2">
              ...
              <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider block">FlowMind Focus Suggestions</span>
            </div>
            <div className="space-y-2.5">
              <div className="text-[10px] text-slate-300 font-semibold p-2.5 rounded-xl bg-slate-950/20 border border-white/[0.02]">
                ⚡ Break down your complex goals into smaller sub-tasks.
              </div>
              <div className="text-[10px] text-slate-300 font-semibold p-2.5 rounded-xl bg-slate-950/20 border border-white/[0.02]">
                ⏱️ Schedule focus times for your high-priority items.
              </div>
            </div>
          </div>
        </div>
